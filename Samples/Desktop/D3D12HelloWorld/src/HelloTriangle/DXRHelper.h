...
IDxcBlob* CompileShaderLibrary(LPCWSTR fileName)
{
  ...
  static dxc::DxcDllSupport libraryhelper;
  static bool libaryInitialized = false;
  if (libaryInitialized == false)
  {
     libraryhelper.Initialize();
  }
  libaryInitialized = true;
  ...
  if (!pCompiler)
  {
    libraryhelper.CreateInstance(CLSID_DxcCompiler, &pCompiler);
    libraryhelper.CreateInstance(CLSID_DxcLibrary, &pLibrary);
